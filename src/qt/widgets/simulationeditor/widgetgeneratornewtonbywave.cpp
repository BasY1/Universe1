/*!
 * \file qt/widgets/simulationeditor/widgetgeneratornewtonbywave.cpp
 * \author Michal Steller
 * \brief Newton wave based simulation initialization data generator widget classes implementation
 */
 ...
 */
Universe1::Widgets::SimulationEditor::WidgetGeneratorNewtonByWave::WidgetGeneratorNewtonByWave(
    Project::QSimulationNewtonByWave *_simulation, QWidget *_parent)
    : QWidget(_parent)
 ...
 * \param _parent Parent \c QWidget
 */
Universe1::Widgets::SimulationEditor::WidgetGeneratorNewtonByWaveUser2::WidgetGeneratorNewtonByWaveUser2(
    Project::QSimulationNewtonByWave *_simulation, QWidget *_parent)
    : WidgetGeneratorNewtonByWave(_simulation, _parent)
    , m_info(new QLabel())
{
    const QSettings settings;
    static const QString key = "SimulationEditor/GeneratorNewtonByWaveUser2/";
    ...
    m_velocity1 =
        new GUI::GuiVector3D(settings.value(key + "velocity1", QVector3D(-1.0F, 1.0F, 0.0F) * 0.1F).value<QVector3D>(),
                             refRangeVel,
                             3,
                             Qt::Horizontal);

    m_velocity2 =
        new GUI::GuiVector3D(settings.value(key + "velocity2", QVector3D(1.0F, -1.0F, 0.0F) * 0.1F).value<QVector3D>(),
                             refRangeVel,
                             3,
                             Qt::Horizontal);
 ...
 */
QString Universe1::Widgets::SimulationEditor::WidgetGeneratorNewtonByWaveUser2::generatorName() const
{
 ...
 */
void Universe1::Widgets::SimulationEditor::WidgetGeneratorNewtonByWaveUser2::rebuild()
{
    m_initObjects.clear();
    ...
    html += TextTools::htmlRow2of3(tr("Velocity ratio [%]"), 100.0L * V1.length() / C, 100.0L * V2.length() / C, decs);
    ...
    html +=
        TextTools::htmlRow2of3(tr("History steps [N]"), moved1.first.timeStamp / DT, moved2.first.timeStamp / DT, decs);
