...
ItemCamera::ItemCamera(const std::string &_name,
    ...
    , position(_name + ".position", _position)
    , lookAt(_name + ".lookAt", _lookAt)
    , up(_name + ".up", true, _up)
    , fieldOfView(_name + ".fieldOfView", _fieldOfView, 5.0, 175.0f)
    , nearPlane(_name + ".nearPlane", _nearPlane, 0.00001f, std::numeric_limits<float>::max())
    , farPlane(_name + ".farPlane", _farPlane, 0.1f, std::numeric_limits<float>::max())
    , imageWidth(_name + ".imageWidth", _imageWidth, 640U, 7680U)
    , imageHeight(_name + ".imageHeight", _imageHeight, 480U, 4320U)
    , imagePixelSize(_name + ".imagePixelSize", _imagePixelSize, 0.0f, std::numeric_limits<float>::max())
    , imagePositionDynamic(_name + ".imagePositionDynamic", _imagePositionDynamic)
    , bodySize(_name + ".bodySize", _bodySize, 0.0f, std::numeric_limits<float>::max())
    ...
    , showImage(_name + ".showImage", _showImage)
    , showBody(_name + ".showBody", _showBody)
    , showViewArrow(_name + ".showViewArrow", _showViewArrow)
    , showViewPyramid(_name + ".showViewPyramid", _showViewPyramid)
    , materialBody1(_name + ".materialBody1", _materialBody1)
    , materialBody2(_name + ".materialBody2", _materialBody2)
