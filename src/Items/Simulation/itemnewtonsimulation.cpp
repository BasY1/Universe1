...
ItemNewtonSimulation::ItemNewtonSimulation(const std::pair<const Math::NewtonSimF *, bool> &_data,
    ...
    , m_dataF(_data.first)
    , time("time", _time, 0.0f, _data.first == nullptr ? 0.0f : _data.first->lastTimeStep())
    , scale("scale", _scale, 0.0f, std::numeric_limits<float>::max())
    , colorMode("colorMode", QMetaEnum::fromType<NewtonSimulation::SimulationColor>(), _colorMode)
    , radiusMode("radiusMode", QMetaEnum::fromType<NewtonSimulation::SimulationRadius>(), _radiusMode)
    , constantSize("constantSize", _constantSize, 1U, MAX_POINT_SIZE)
    , maxPointSize("maxPointSize", _maxPointSize, 1U, MAX_POINT_SIZE)
    , constantColor("constantColor", _constantColor)
    , positiveColor("positiveColor", _positiveColor)
    , negativeColor("negativeColor", _negativeColor)
    , dynamicShine("dynamicShine", _dynamicShine)
    , dynamicDistMaxShine("dynamicDistMaxShine", _dynamicDistMaxShine, 0.0f, std::numeric_limits<float>::max())
    , dynamicDistMinShine("dynamicDistMinShine", _dynamicDistMinShine, 0.0f, std::numeric_limits<float>::max())
    , dynamicRadius("dynamicRadius", _dynamicRadius)
    , dynamicDistMaxRadius("dynamicDistMaxRadius", _dynamicDistMaxRadius, 0.0f, std::numeric_limits<float>::max())
    , dynamicDistMinRadius("dynamicDistMinRadius", _dynamicDistMinRadius, 0.0f, std::numeric_limits<float>::max())
{
    addProperty(&time);
    addProperty(&scale);
...
ItemNewtonSimulation::ItemNewtonSimulation(const std::pair<const Math::NewtonSimD *, bool> &_data,
    ...
    , m_deleteSimulation(_data.second)
    , m_dataD(_data.first)
    , time("time", _time, 0.0f, _data.first == nullptr ? 0.0f : float(_data.first->lastTimeStep()))
    , scale("scale", _scale, 0.0f, std::numeric_limits<float>::max())
    , colorMode("colorMode", QMetaEnum::fromType<NewtonSimulation::SimulationColor>(), _colorMode)
    , radiusMode("radiusMode", QMetaEnum::fromType<NewtonSimulation::SimulationRadius>(), _radiusMode)
    , constantSize("constantSize", _constantSize, 1U, MAX_POINT_SIZE)
