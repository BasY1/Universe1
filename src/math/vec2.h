...
#ifndef UNIVERSE1_MATH_VEC2_H
#define UNIVERSE1_MATH_VEC2_H
...
struct Vec2
{
    ...
    inline bool isSameDir(const Vec2<T> &_other) const;
    inline bool isParallel(const Vec2<T> &_other) const;
    inline bool isParallelInDir(const Vec2<T> &_other, bool &_outIsSameDirection) const;
...
{
    return isSame(x, _other.x) && isSame(y, _other.y);
}
...
{
    return isZero(Vec2<T>::dot(*this, _other));
}
 ...
 */
template <typename T>
inline bool Vec2<T>::isSameDir(const Vec2<T> &_other) const
{
 ...
 * \tparam T Template floating point type
 * \param _other Other vector
 * \param[out] _outIsSameDirection Output is same direction flag
 * \return \c true if vectors are parallel
 */
template <typename T>
inline bool Vec2<T>::isParallelInDir(const Vec2<T> &_other, bool &_outIsSameDirection) const
{
    if (isNull() || _other.isNull())
    {
        _outIsSameDirection = false;
        return false;
    }
    const Vec2<T> n1 = normalized();
    const Vec2<T> n2 = _other.normalized();
    _outIsSameDirection = (n1 == n2);
    return _outIsSameDirection || (n1 == -n2);
}
...
/*!
 * \brief Converts vector to text in fixed decimal count format
 * \tparam T Template floating point type
 * \param _decimals Fixed decimal count
 * \return Vector as text string
 */
template <typename T>
std::string Vec2<T>::toStringFull(const int _decimals) const
{
    std::stringstream ss;
    if (_decimals >= 0)
    {
