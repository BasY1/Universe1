...
/*! \brief Audio scenario for sounds generated by \b sox tool */
class ScenarioAudioSox : public ScenarioAudio
{
 public:
    const QString workDir;         //!< Working directory for generated audio files
    const SettingsAudio settings;  //!< Audio settings
     ...
     * \param _settings Audio settings
     */
    ScenarioAudioSox(const std::string &_name, const QString &_workDir, const SettingsAudio &_settings);

    /*!
     * \brief Append sound generated by \b sox into this scenario
     * \param _timeStart Start time step
     * \param _soxArgs \b sox tool arguments
     * \return Success flag
     */
    bool insertSound(const size_t _timeStart, const QStringList &_soxArgs);

    /*!
     * \brief Append sound generated by \b sox to the end of this scenario
     * \param _timeWait Time wait after latest active time-step
     * \param _soxArgs \b sox tool arguments
     * \return Success flag
     */
    inline bool addSound(const size_t _timeWait, const QStringList &_soxArgs)
    {
        return insertSound(latestTimeStep() + _timeWait, _soxArgs);
    }
};
...
class ScenarioAudioTTS : public ScenarioAudio
{
 public:
     ...
     * \brief Append speech audio to the end of this scenario
     * \param _timeWait Time wait after latest active time-step
     * \param _speechTextAndSubs Text used for speech and subtitles
     * \param _offsetSubs Subtitles overlay duration
     * \return Success flag
     */
    inline bool addSpeechSubs(const size_t _timeWait, const QString &_speechTextAndSubs, const size_t _offsetSubs = 0U)
    {
        return addSpeech(_timeWait, _speechTextAndSubs, _speechTextAndSubs, _offsetSubs);
    }
     ...
     * \param _offsetSubs Subtitles overlay duration
     * \return Success flag
     * \note Subtitles are converted to upper-case
     */
    inline bool addSpeechSUBS(const size_t _timeWait, const QString &_speechTextAndSubs, const size_t _offsetSubs = 0U)
    {
        return addSpeech(_timeWait, _speechTextAndSubs, _speechTextAndSubs.toUpper(), _offsetSubs);
    }
