...
/*!
 * \brief Circle step angle for given circle quality
 * \param _quality Circle quality
 * \return Circle step angle in radians
 */
template <typename T, typename = std::enable_if<std::is_floating_point<T>::value>>
inline T circleStepAngle(const size_t _quality)
{
 ...
 */
inline size_t sphereVertexCount(const size_t _quality)
{
 ...
 */
inline size_t sphereIndexCount(const size_t _quality)
{
    const size_t cntCircle = circlePointCount(_quality);
    const size_t cntLon = cntCircle;
    const size_t cntLat = cntCircle / 2UL;
